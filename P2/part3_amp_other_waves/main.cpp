...
int main(){
	...
	for(int n=0; n<N; n++){
		// Clipping sine
		//rawData[n] = oscillator_sine(phase, freq, sr) * 1.4f;	
		
		// Square wave
		//rawData[n] = oscillator_square(phase, freq, sr);
		
		// Saw wave
		//rawData[n] = oscillator_saw(phase, freq, sr);
		
		// Triangle wave
		rawData[n] = oscillator_triangle(phase, freq, sr);
	}
		
	WaveFileHeader header(N, channels, sr, sizeof(int16_t));
	write_wavfile<float, int16_t>("triangle.wav", header, rawData, N);	  
	std::cout << "Done!\n" << std::endl;
